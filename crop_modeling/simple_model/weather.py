...
"""
THIS PENMAN MONTEITH IMPLEMENTATION WAS TAKEN FROM THE FOLLOWING REPOSITORY
https://github.com/sherzodr/penmon/blob/master/src/penmon/eto.py
...
class Station:
    ...
    def __init__(self, latitude, longitude, altitude, anemometer_height=2):
        """
        Required parameters:
...
class DayEntry:
    ...
    **Station()**'s day_entry() method, passing all reuqired state data.
    ...
    def relative_sun_distance(self, days_of_year = None):
        """
        Eq. 23
        """
        if days_of_year is None: days_of_year = 365
        
        return round(1 + 0.033 * math.cos((2 * math.pi / days_of_year) * self.day_number), 3)
    ...
    def solar_radiation(self):
        ...
        if self.radiation_s:
            ...
            if CHECK_RADIATION_RANGE and (self.radiation_s > self.R_so()+10):
                raise ValueError(
...
def petpt(msalb, srad, tmax, tmin, xhlai):
    ...
    """
    Priestley-Taylor Evapotranspiration implementation

    """
    td = 0.6 * tmax + 0.4 * tmin

    if xhlai<= 0: albedo = msalb
    else: albedo = 0.23- (0.23-msalb) * math.exp(-0.75* xhlai)
    slang = srad * 23.923
    eeq = slang * (2.04E-4 - 1.83E-4 * albedo) * (td + 29.0)
    pt = eeq * 1.1
    if tmax > 35: pt = eeq * ((tmax - 35.0) * 0.05 + 1.1)
    elif tmax < 5.0: pt = eeq * 0.01 * math.exp(0.18 * (tmax + 20))
    if pt < 0.0001: pt = 0.0001
...
def etpm(self):
    ws2 = self.day_weather.wind_speed_2m()
    slope = self.day_weather.slope_of_saturation_vapour_pressure(self.day_weather.Tmean())
    es = self.day_weather.mean_saturation_vapour_pressure()
    #TODO LEAP YEAR
    irdes = self.day_weather.relative_sun_distance()
    sd = self.day_weather.solar_declination()
    ssa = self.day_weather.sunset_hour_angle()
    extra = self.day_weather.R_a()
    # 24*60*0.082/pi*IRDES*(SSA*sin(lat)*sin(SD)+cos(lat)*cos(SD)*sin(SSA)) TODO CHECK        
    csr = self.day_weather.R_so()
    rrad = self.srad/csr
...
class Weather():
    ...
    def get_day_eto(self, eto_method = 'PM'):
        ...
        elif self.eto_method == 'PT':
            albedo = 0.23
            etoi = petpt(albedo, self.srad, self.tmax, self.tmin, -99)
        return etoi
    ...
    def get_day_weather(self, day, meteorological_variables = None, day_of_year_col_name = 'julian_day'):
        ...
        dayweather =  self.weather.iloc[day]
        
        val = {}
        for i in meteorological_variables:
            val[i] = dayweather[i]
            setattr(self, i, val[i])
            
        setattr(self, day_of_year_col_name, int(dayweather[day_of_year_col_name]))
         
        self._set_day_meteorological_vars()
